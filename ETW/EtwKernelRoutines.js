...
function initializeScript()
{
    return [new host.functionAlias(AllRegisteredLoggersWithConsumers, "EtwLoggersAndConsumers"),
            ...
            new host.functionAlias(EtwConsumersForProcessByName, "EtwConsumersForProcessByName"),
...
// --- NEW FUNCTION ---
function EtwConsumersForProcessByName(targetName)
{
    let dbgOutput = host.diagnostics.debugLog;
    let processes = host.currentSession.Processes;
    let found = false;

    // Case-insensitive comparison since process names in the kernel
    // are stored as-is (e.g. "MsMpEng.exe", "svchost.exe")
    ...
    for (let process of processes)
    {
        try
        {
            let procName = process.Name.toLowerCase();
            if (procName === lowerTarget || procName.indexOf(lowerTarget) !== -1)
            {
                found = true;
                EtwConsumersForProcess(process);
                dbgOutput("\n");
            ...
            dbgOutput("Exception checking process name: ", e, "\n");
        }
    }

    if (!found)
    {
        dbgOutput("No process matching '", targetName, "' was found.\n");
    }
}
