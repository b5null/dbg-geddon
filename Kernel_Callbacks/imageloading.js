...
function resolvePspImageCallbacks() {
    const log = x => host.diagnostics.debugLog(x + "\n");
    const ctl = host.namespace.Debugger.Utility.Control;

    log("[*] PspCreateImageNotifyRoutine callbacks:\n");

    let results = ctl.ExecuteCommand("dqs nt!PspLoadImageNotifyRoutine L10");
...
function initializeScript() {
    return [
        new host.functionAlias(resolvePspImageCallbacks, "resolvePspImageCallbacks")
    ];
}
