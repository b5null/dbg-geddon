...
function resolvePspThreadCallbacks() {
    const log = x => host.diagnostics.debugLog(x + "\n");
    const ctl = host.namespace.Debugger.Utility.Control;

    log("[*] PspCreateThreadNotifyRoutine callbacks:\n");

    let results = ctl.ExecuteCommand("dqs nt!PspCreateThreadNotifyRoutine L10");
...
function initializeScript() {
    return [
        new host.functionAlias(resolvePspThreadCallbacks, "resolvePspThreadCallbacks")
    ];
}
